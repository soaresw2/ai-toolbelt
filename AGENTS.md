...
Each folder contains exactly **one README.md** (guidelines) and **one example file** prefixed with `_example-` (reference implementation only -- not for direct use). Do not add a second example file to any folder without explicit approval. New files contributed by users should **not** use the `_example-` prefix.
